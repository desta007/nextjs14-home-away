    ...
    <div className="grid grid-cols-2 gap-4">
        {selectedAmenities.map((amenity)=>{
            return <div key={amenity.name} className="flex items-center space-x-2">
                ...
                <label htmlFor={amenity.name} className="text-sm font-medium leading-none capitalize flex 
                gap-x-2 items-center">{amenity.name} <amenity.icon className="w-4 h-4" /></label>
            </div>
        })}
    </div>
  </section>
